...
public class MenuView{
	...
	*/
	public void printMenu(){
		 ...
		 while(true){
	           System.out.println("----------------------------------------------------------------------------------");
	           System.out.println("1. 전체검색    2. 모델번호검색     3.등록     4. 수정   5. 삭제   6.가격기준정렬  7.할인적용  8.일치하는 이름 검색  9. 종료");
			   System.out.println("----------------------------------------------------------------------------------");
  ...
  /**
   * 할인 적용하기 위해 키보드 입력 처리하는 메소드
   */
  public void inputDiscont() {
	  System.out.println("할인을 적용시킬 모델 번호는?");
	  int modelNo = Integer.parseInt(sc.nextLine());
	  System.out.println("적용시킬 할인율은?");
	  int discountPercent = Integer.parseInt(sc.nextLine());
	  controller.discountModelNo(modelNo, discountPercent);
  }
  ...
  public void inputSearchName() {
	  System.out.println("검색할 모델 이름은?");
	  String modelName = sc.nextLine();
	  controller.searchModelName(modelName);
  }
}
