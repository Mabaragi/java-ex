  ...
  public void inputInsert(){
	 ...
	 //생성자를 추가하여 값을 전달하자(데이터 초기화)
      Goods goods = new Goods(code, name, price, explain);
	   ...
	   InsertResponse result = service.insert(goods);

	   switch(result){
		case OUT_OF_RANGE -> EndView.printMessage("더이상 등록할 수 없습니다.");
		case DUPULICATED -> EndView.printMessage(code+"는 중복이므로 등록할수 없습니다.");
		case SUCCESS -> EndView.printMessage("상품이 등록되었습니다.");
	   }
  }
  ...
  */
  public void inputUpdate(){
	   ...
	   //위 3개의 정보를 하나의 Goods객체로 만든다.
		Goods goods = new Goods(code, null, price, explain);
